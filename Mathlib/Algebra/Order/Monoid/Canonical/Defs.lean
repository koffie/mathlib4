...
class CanonicallyOrderedAddMonoid (α : Type _) extends OrderedAddCommMonoid α, OrderBot α where
  /-- For `a ≤ b`, there is a `c` so `b = a + c`. -/
...
-/
@[to_additive]
class CanonicallyOrderedMonoid (α : Type _) extends OrderedCommMonoid α, OrderBot α where
  /-- For `a ≤ b`, there is a `c` so `b = a * c`. -/
